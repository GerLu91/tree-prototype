...
import { 
  ...
  ArrowUp, ArrowDown, Trash2, Plus, GripVertical, Route 
} from "lucide-react";
...
interface TaskListProps {
  ...
  // NEUE PROPS FÜR TOUR-MANAGEMENT
  tour: string[];
...
export function TaskList({ 
  ...
  // 1. Tour-Stopps finden (Bäume in der Tour)
  const tourStops = tour.map(id => trees.find(t => t.id === id)).filter(Boolean) as Tree[];

  // 2. Offene Aufgaben finden, die NOCH NICHT in der Tour sind
  const availableTasks = tasks.filter(task => 
    task.status !== 'erledigt' && !tour.includes(task.treeId)
  );

  // Hilfsfunktion zum Umordnen
  ...
  return (
    <div className="p-4 space-y-8 max-w-md mx-auto pb-32">
      ...
      <section className="space-y-4">
        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-600 flex items-center gap-2">
          ...
          Aktuelle Tour ({tour.length} Stopps)
        </h3>
        ...
        <div className="space-y-3">
          {tourStops.length > 0 ? (
            tourStops.map((tree, index) => {
              ...
              return (
                <Card key={tree.id} className="border-none shadow-md bg-white overflow-hidden ring-1 ring-slate-100">
                  <div className="flex">
                    {/* Index & Sortierung */}
                    <div className="w-12 bg-slate-50 border-r border-slate-100 flex flex-col items-center justify-center gap-2 py-2">
                      <button 
                        ...
                        disabled={index === 0}
                        className="text-slate-400 disabled:opacity-20 active:text-blue-600"
                      >
                        <ArrowUp size={16} />
                      </button>
                    ...
                    <div className="flex-1 p-4">
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="font-bold text-slate-900 leading-tight">{tree.species}</h4>
                      ...
                      <div className="mt-4 flex justify-between items-center">
                        ...
                        <Button 
                          ...
                          className="rounded-xl h-8 bg-blue-600 hover:bg-blue-700 shadow-md shadow-blue-100 text-[9px] font-black uppercase"
                        >
            ...
            <div className="text-center py-8 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
              ...
              <p className="text-[10px] text-slate-400 mt-1">Wähle Aufgaben aus oder nutze die Karte</p>
            </div>
          )}
        </div>
      </section>
          ...
          <div className="space-y-2">
            {availableTasks.map(task => {
              ...
              return (
                <div 
                  key={task.id} 
                  className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl border border-slate-100 group active:bg-white transition-colors"
                >
                  ...
                  <Button 
                    variant="outline" 
                    size="sm" 
                    className="h-8 rounded-xl border-slate-200 text-[9px] font-black uppercase hover:bg-blue-600 hover:text-white hover:border-transparent transition-all"
      ...
      <div className="bg-amber-50 p-4 rounded-2xl border border-amber-100 flex gap-3">
        ...
        <p className="text-[10px] text-amber-700 leading-relaxed font-medium">
          <strong>Pro-Tipp:</strong> Du kannst Bäume auch direkt in der <strong>Karten-Ansicht</strong> über das Routen-Symbol zur Tour hinzufügen.
        </p>
      </div>
    </div>
  );
